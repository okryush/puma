...
def load_velo_scan(file):
    """Load and parse a velodyne pcd file."""
    pc = o3d.io.read_point_cloud(file)
    scan = np.asarray(pc.points)
    return scan.reshape((-1, 3))
...
def main(dataset, out_dir, sequence, use_intensity):
    ...
    """
    print(
        "Converting .pcd scans into .ply fromat from:{orig} to:{dest}".format(
    ...
    if sequence:
        ...
        os.makedirs(base_path, exist_ok=True)
        velo_files = sorted(glob.glob(os.path.join(dataset, "lidar0_*.pcd")))
